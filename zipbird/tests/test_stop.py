...
class TestStopOrder(unittest.TestCase):
    ...
    def test_percent_trailing_stop(self):
        stop = PercentTrailingStop(LongShort.Long, 100, 0.1)
    ...
    def test_percent_trailing_short(self):
        stop = PercentTrailingStop(LongShort.Short, 100, 0.1)
        stop.update_stop_price({'close': 100})
    ...
    def test_atr_trailing_stop_long(self):
        stop = ATRTrailingStop(LongShort.Long, 100, 2)
    ...
    def test_atr_trailing_stop_short(self):
        stop = ATRTrailingStop(LongShort.Short, 100, 2)
        stop.update_stop_price({'close': 100, 'atr': 3})
    ...
    def test_stop_order_trailing_stop_long(self):
        trailing = ATRTrailingStop(LongShort.Long, 120, 2)
        stop = StopOrder(trailing=trailing)
        # has not passed entry price, no trigger either way
        stop.update_stops({'close': 100, 'atr': 3})
        self.assertEqual(stop.get_status({'close': 100}), StopOrderStatus.NOT_TRIGGER)
        self.assertEqual(stop.get_status({'close': 93}), StopOrderStatus.NOT_TRIGGER)

        # passed entry price
        stop.update_stops({'close': 200, 'atr': 3})
        self.assertEqual(stop.get_status({'close': 200}), StopOrderStatus.NOT_TRIGGER)
        self.assertEqual(stop.get_status({'close': 193}), StopOrderStatus.TRAILING_STOP)

    def test_stop_order_trailing_stop_short(self):
        trailing = ATRTrailingStop(LongShort.Short, 90, 2)
        stop = StopOrder(trailing=trailing)
        # has not passed entry price, no trigger either way
        stop.update_stops({'close': 100, 'atr': 3})
        self.assertEqual(stop.get_status({'close': 100}), StopOrderStatus.NOT_TRIGGER)
        self.assertEqual(stop.get_status({'close': 107}), StopOrderStatus.NOT_TRIGGER)

        # passed entry price
        stop.update_stops({'close': 80, 'atr': 3})
        self.assertEqual(stop.get_status({'close': 83}), StopOrderStatus.NOT_TRIGGER)
    ...
    def test_stop_order_multiple_stops1(self):
        trailing = ATRTrailingStop(LongShort.Long, 100, 4)
    ...
    def test_stop_order_multiple_stops_short(self):
        trailing = ATRTrailingStop(LongShort.Short, 100,  4)
